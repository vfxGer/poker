...
from cli.players import AIPlayerInterface, CLIPlayer


from poker_core.core_game import Core_Game
from defaults import DEFAULT_MIN_BET, DEFAULT_NUM_PLAYERS

class CLI_Game(Core_Game):
    def __init__(self):
        self.min_bet = DEFAULT_MIN_BET
        Core_Game.__init__(self)

    def init_players(self):
        players = OrderedDict({})
        for i in xrange(DEFAULT_NUM_PLAYERS-1):
            name = "Player_%d" % i
            players[name] = AIPlayerInterface()
